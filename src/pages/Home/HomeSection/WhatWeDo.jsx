...
const WhatWeDo = () => {
  ...
  return (
    <div className=' pt-45 w-full '>
        <div className=' px-10 w-full '>
            <h3 className='Switzer-light  text-[1.2vw] uppercase '>What we do</h3>
            <h2 className='Switzer-light tracking-tight text-[4.3vw] leading-19 w-5xl py-5'>We are artists, designers, <br />
...
real-world spaces.</h2>
        ...
        <div className=' flex items-center justify-center w-full'>
            <div className='h-full w-fit'>
                <h4 className='py-12 text-2xl tracking-tight w-4xl text-[#9D9C9A] Switzer-light'>We go beyond the traditional agency model and empower people across different
...
<div className="flex flex-col md:flex-row gap-3 h-full w-full">
      {data.map((item, index) => (
        <div key={index} className="flex flex-col h-full w-64">
          ...
          <p className="Switzer-light tracking-tighter text-lg md:text-1xl mt-2">{item.text}</p>
        </div>
      ))}
    </div>
